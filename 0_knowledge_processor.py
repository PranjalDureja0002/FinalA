...
def _build_column_values(data):
    ...
    Normalizes into the format the pipeline expects:
    { "COLUMN_NAME": { "cardinality": N, "examples": [...], "complete": bool } }
    """
    if not data:
        return {}
    raw = data
    ...
    for col_name, col_info in raw.items():
        if isinstance(col_info, dict):
            # Already structured: { cardinality: N, examples: [...] }
            examples = col_info.get("examples", col_info.get("values", []))
            # Handle list of dicts (from profiler: [{"value": "X", "count": 100}, ...])
...
class CodeEditorNode(Node):
            ...
            for line in text_lines:
                ...
                if content:
                    ...
                    if parsed is not None:
                        ...
                        # Parsing failed — check if filename matches a text-based slot
                        # (data_context.txt is free text, not YAML/JSON)
                        norm = _normalize_filename(filename)
                        for keywords, slot in _FILENAME_KEYWORDS:
                            if any(kw in norm for kw in keywords):
                                if slot == "data_context_file":
                                    # Store raw text content directly
    ...
    def build_output(self) -> Data:
        ...
        # data_context_file → inject into additional_domain_context
        data_context_raw = slots.get("data_context_file")
        if data_context_raw:
            # data_context.txt is free text — pass through as-is
            if isinstance(data_context_raw, dict):
                # If it was parsed as YAML/JSON, convert back to text
        ...
        self.status = f"Loaded: {loaded}/14 | {len(synonym_map)} synonyms, {len(entities)} entities, {len(examples)} examples"

        # Summary text
        parts = [f"**Knowledge Processor** (pure code -- no LLM)\n", f"**Files Detected:** {loaded}/14"]
        ...
        for label, count in [("Synonyms", len(synonym_map)), ("Entities", len(entities)),
                             ...
                             ("Data Context", 1 if slots.get("data_context_file") else 0)]:
            if count > 0:
